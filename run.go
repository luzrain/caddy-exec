...
func (c *Cmd) run(params []string) error {
	cmdInfo := zap.Any("command", append([]string{c.Command}, params...))
	log := c.log.With(cmdInfo)
	startTime := time.Now()

	cmd := exec.Command(c.Command, params...)
	...
	if c.timeout > 0 {
		...
		cmd = exec.CommandContext(ctx, c.Command, params...)
	}
